...
class MatchGame extends Component {
  ...
  onClickImage = id => {
    if (this.randomImgId === id) {
      ...
      console.log(this.randomImgCategory)
      console.log(id)
    } else {
      this.setState({
        gameOver: true,
  ...
  render() {
    const {score, time, tabId, gameOver} = this.state

    return (
      <div className="container">
        <HeaderPart score={score} remainingTime={time} />
        {gameOver === false ? (
          <>
            <img className="given-img" src={this.randomImg} alt="match" />
